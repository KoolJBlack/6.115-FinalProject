...
void UARTPrintFFTBeatInfo8051Amulet(uint8 mode){
    //char output[32];  //C character output buffer
    UART_PutChar(0xFF); // Indicates start of output
    int i;
    uint8 output;
...
void ProcessAmulet(uint8 input){
    //LCD_PutChar('j');     // Print received character to LCD
    switch(input){
        case 0x30:  // FFT Bars
            UARTPrintFFTBeatInfo8051Amulet(0x17);
            CyDelay(500);
            break;
        case 0x31:  // Beat Detect
            UARTPrintFFTBeatInfo8051Amulet(0x16);
            CyDelay(500);
            break;
        case 0x32: // Beat Pattern
            UARTPrintFFTBeatInfo8051Amulet(0x15);
            CyDelay(500);
            break;
...
void Init() {
    ...
    rx_int_Amulet_Start();                     // start RX interrupt--look at this file under "Generated_Source"
                                        // for code that writes received bytes to LCD.
    
    UART_Start();                       // initialize UART
    UART_ClearRxBuffer();
    
    UART_Amulet_Start();                       // initialize UART
